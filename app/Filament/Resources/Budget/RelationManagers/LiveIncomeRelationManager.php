...
class LiveIncomeRelationManager extends RelationManager
{
    protected static string $relationship = 'incomeEntries';

    protected static ?string $title = 'Live Income';

    public function table(Table $table): Table
    {
        return $table
            ->modifyQueryUsing(fn ($query) => $query->where('is_live', true))
            ->columns([
            ...
            ->headerActions([
                CreateAction::make()
                    ->label('Add Live Income')
                    ->mutateFormDataUsing(fn (array $data) => array_merge($data, ['is_live' => true, 'type' => 'other']))
    ...
    private function incomeFormSchema(): array
    {
        return [
            TextInput::make('label')
                ...
                ->required()
                ->maxLength(100)
                ->helperText('e.g. Bank, Cash in Hand, Tax Return, Cheque')
                ->columnSpan(2),
